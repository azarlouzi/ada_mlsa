...
def plot_time_vs_rmse(title:str, algorithm_list:Tuple[str], accuracy_dict:Dict[int,float],
    ...
    labels = []
    for cohort in range(1, len(accuracy_dict)+1):
        accuracy = accuracy_dict[cohort]
        numerator = math.ceil(accuracy)
        denominator = math.ceil(1./accuracy)
        if denominator > 1:
            labels.append(r"$\frac{%d}{%d}$" %(numerator, denominator))
        else:
            labels.append(r"%d" %numerator)
...
def sub_process(filename:str, outpath:str, VaR:float)->None:
    ...
    time_ada_nsa_nosat_dict = defaultdict(list)
    time_ada_mlsa_nosat_dict = defaultdict(list)
    ...
    estimate_VaR_ada_nsa_nosat_dict = defaultdict(list)
    estimate_VaR_ada_mlsa_nosat_dict = defaultdict(list)
    ...
    with open(filename, "r") as instream:
        ...
        for row in reader:
            ...
            time_ada_nsa_nosat_dict[cohort].append(float(d["time_ada_nsa_nosat"]))
            time_ada_mlsa_nosat_dict[cohort].append(float(d["time_ada_mlsa_nosat"]))
            ...
            estimate_VaR_ada_nsa_nosat_dict[cohort].append(float(d["VaR_ada_nsa_nosat"]))
            estimate_VaR_ada_mlsa_nosat_dict[cohort].append(float(d["VaR_ada_mlsa_nosat"]))
    ...
    average_time_ada_nsa_nosat = []
    average_time_ada_mlsa_nosat = []
    ...
    rmse_VaR_ada_nsa_nosat = []
    rmse_VaR_ada_mlsa_nosat = []
    ...
    for cohort in range(1, n_cohort+1):
        ...
        average_time_ada_nsa_nosat.append(average(time_ada_nsa_nosat_dict[cohort]))
        average_time_ada_mlsa_nosat.append(average(time_ada_mlsa_nosat_dict[cohort]))
        ...
        rmse_VaR_ada_nsa_nosat.append(rmse(estimate_VaR_ada_nsa_nosat_dict[cohort], VaR))
        rmse_VaR_ada_mlsa_nosat.append(rmse(estimate_VaR_ada_mlsa_nosat_dict[cohort], VaR))
    ...
    VaR_ada_nsa_nosat_slope, VaR_ada_nsa_nosat_fit = fit_y_vs_x(average_time_ada_nsa_nosat, rmse_VaR_ada_nsa_nosat)
    VaR_ada_mlsa_nosat_slope, VaR_ada_mlsa_nosat_fit = fit_y_vs_x(average_time_ada_mlsa_nosat, rmse_VaR_ada_mlsa_nosat)
    ...
    print("VaR adNSA complexity exponent: %f" %VaR_ada_nsa_slope)
    print("VaR adNSA SD complexity exponent: %f" %VaR_ada_nsa_sd_slope)
    print("VaR adMLSA complexity exponent: %f" %VaR_ada_mlsa_slope)
    print("VaR adMLSA SD complexity exponent: %f" %VaR_ada_mlsa_sd_slope)
    print("VaR unsaturated adNSA complexity exponent: %f" %VaR_ada_nsa_nosat_slope)
    print("VaR unsaturated adMLSA complexity exponent: %f" %VaR_ada_mlsa_nosat_slope)
    print("\n")
    ...
    VaR_ada_nsa_nosat_slope, VaR_ada_nsa_nosat_accuracy_fit = fit_y_vs_x(average_time_ada_nsa_nosat, accuracies)
    VaR_ada_mlsa_nosat_slope, VaR_ada_mlsa_nosat_accuracy_fit = fit_y_vs_x(average_time_ada_mlsa_nosat, accuracies)
    ...
    print("VaR adMLSA complexity exponent: %f" %VaR_ada_mlsa_slope)
    print("VaR adMLSA SD complexity exponent: %f" %VaR_ada_mlsa_sd_slope)
    print("VaR unsaturaed adNSA complexity exponent: %f" %VaR_ada_nsa_nosat_slope)
    print("VaR unsaturaed adMLSA complexity exponent: %f" %VaR_ada_mlsa_nosat_slope)
    print("\n")
    ...
    summary = {
      "SA": {
         ...
         "time": average_time_ada_nsa_nosat,
         "rmse": rmse_VaR_ada_nsa_nosat,
         "time_fit": VaR_ada_nsa_nosat_fit,
         "accuracy_time_fit": VaR_ada_nsa_nosat_accuracy_fit,
         "symbol": "-h",
         "symbol_size": 12,
         "color": "xkcd:turquoise",
      }, "u-adMLSA": {
         "time": average_time_ada_mlsa_nosat,
         "rmse": rmse_VaR_ada_mlsa_nosat,
         "time_fit": VaR_ada_mlsa_nosat_fit,
         "accuracy_time_fit": VaR_ada_mlsa_nosat_accuracy_fit,
         "symbol": "-+",
         "symbol_size": 12,
         "color": "xkcd:red",
      },
...
if __name__ == "__main__":
    ...
    p.add_argument("--test_case", help="Test set to process", choices=["1","2"])
    args = p.parse_args()
